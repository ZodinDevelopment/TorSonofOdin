...
class User(UserMixin, db.Model):
    ...
    def generate_token(self):
        ...
        token_string = ':'.join([secret, self.username, self.email, self.password_hash])

        self.auth_token = md5(token_string.encode('utf-8')).hexdigest()
    ...
    def followed_videos(self):
        followed = Video.query.join(
            followers, (followers.c.followed_id == Video.user_id)).filter(
                followers.c.follower_id == self.id)
        own = Video.query.filter_by(user_id=self.id)

        return followed.union(own).order_by(Video.timestamp.desc())
